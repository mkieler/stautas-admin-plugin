...
//variables
$GLOBALS["plugin_url"] = plugin_dir_url( __FILE__ );
...
function load_custom_wp_admin_style($hook) {
	if (str_contains($hook, 'stautas-admin')) {
		wp_enqueue_style( 'stautas_plugin_style', $GLOBALS["plugin_url"] . 'style.css' );
		wp_enqueue_style( 'stautas_all_clients_style', $GLOBALS["plugin_url"] . 'assets/style/all-clients.css' );
		wp_enqueue_style( 'create-client-style', $GLOBALS["plugin_url"] . 'assets/style/create-client.css' );
	}	
}
add_action( 'admin_enqueue_scripts', 'load_custom_wp_admin_style' );

wp_enqueue_style( 'ajax_post_message_style', $GLOBALS["plugin_url"] . 'assets/style/ajax-post-message.css' );
wp_enqueue_style( 'font-awesome', 'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css' );
wp_enqueue_style( 'loading-screen-style', $GLOBALS["plugin_url"] . 'assets/style/loading-screen.css' );
...
wp_enqueue_script( 'create-client-ajax', $GLOBALS["plugin_url"] . '/assets/js/create-client-ajax.js', array( 'jquery' ), false, false );
wp_enqueue_script( 'create-sub-client-ajax', $GLOBALS["plugin_url"] . '/assets/js/create-sub-client-ajax.js', array( 'jquery' ), false, false );


if (is_admin()) {
wp_enqueue_script( 'stautas_admin_navigation', $GLOBALS["plugin_url"] . 'assets/js/navigation.js', false, false );
wp_enqueue_script( 'settings-ajax', $GLOBALS["plugin_url"] . '/assets/js/settings-ajax.js', array( 'jquery' ), false, false );
...
function myPluginCreateTable() {
	$contents = file_get_contents($GLOBALS["plugin_url"] . "API/DBAccess/initDB.php");
}
...
function includeAdminMenuFiles(){
	//include files	
	include_once( __DIR__ . '/set-admin-pages.php' );
}
...
include_once( __DIR__ . '/register-elementor-tags.php' );
...
{
   $templates[str_replace("\\","/", __DIR__) . '/templates/userpanel-template.php'] = __( 'Brukerpanel', 'text-domain' );
...
function pt_change_page_template($template){ 
    $userpanel_template = str_replace("\\","/", __DIR__) . '/templates/userpanel-template.php';
